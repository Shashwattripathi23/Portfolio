...
export default function App({ projects = DEFAULT_PROJECTS, state = 'hero', onHeroClick }: MonitorProps) {
  ...
  useEffect(() => {
    ...
    if (state === 'hero') {
      interval = setInterval(() => {
        if (!isHoveredRef.current && activeOverlay === 'none') {
          setIsExploded(true);
  ...
  const DevProjectCard = ({ project }: { project: any }) => {
    ...
    return (
      <div className="flex flex-col items-center gap-1.5 p-2 rounded-xl group hover:bg-white/5 transition-colors pointer-events-auto cursor-pointer w-full">
  ...
  const MLProjectRow = ({ project, idx }: { project: any; idx: number }) => {
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-2 hover:bg-white/[0.03] transition-colors border-b border-zinc-800/40 group pointer-events-auto cursor-pointer">
